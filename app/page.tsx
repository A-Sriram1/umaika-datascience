...
import { Search, Share2, MessageCircle, ArrowRight, CheckCircle, Factory, Brain, Cog, Zap, UserPlus, Globe, Send, Mail, ChevronRight, ChevronLeft } from 'lucide-react';
...
export default function MagazineTemplatePage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800 font-sans selection:bg-[#1b4d3e] selection:text-white">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-xs">
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2">
            <span className="text-3xl font-serif italic text-[#1b4d3e] font-extrabold tracking-tight">FactoryMind <span className="text-[#c48b71]">Journal</span></span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden sm:block">
              ...
              <input type="text" placeholder="Search..."
                className="pl-9 pr-4 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#1b4d3e]" />
            </div>
      ...
      <section className="relative bg-[#1b4d3e] py-12 px-6 overflow-hidden">
        {/* Background Image Overlay */}
        <div className="absolute inset-0 opacity-20">
          <Image src="/images/smart_factory.jpg" alt="Smart Factory Background" fill className="object-cover" />
        </div>

        {/* Carousel Navigation Arrows & Container */}
        <div className="relative max-w-5xl mx-auto flex items-center justify-between mb-8">
          <button className="w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 text-white flex items-center justify-center backdrop-blur-md transition-all">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <div className="text-center text-white px-4">
            <h1 className="text-3xl md:text-5xl font-serif font-extrabold tracking-tight">AI Smart Manufacturing Journal</h1>
            <p className="text-emerald-100 text-xs md:text-sm mt-2 max-w-xl mx-auto font-medium">
        ...
        {/* 4 Overlapping Circular Cards */}
        <div className="relative max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 pt-4">
          {carouselItems.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <Link href={item.route} className="flex flex-col items-center">
                <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white p-1 shadow-2xl bg-white overflow-hidden group-hover:scale-105 transition-transform duration-300 relative">
                  <Image src={item.img} alt={item.title} fill className="object-cover rounded-full" />
                </div>
                <span className="mt-3 text-xs md:text-sm font-extrabold text-white bg-slate-950/70 px-4 py-1.5 rounded-full backdrop-blur-md border border-white/20 group-hover:bg-[#1b4d3e] transition-colors">
                  {item.title}
                </span>
              </Link>
            </motion.div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          {/* Left Column (Features Section - 2/3 width) */}
          <div className="lg:col-span-2 space-y-10">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs items-center">
              ...
              <div className="flex flex-col justify-between h-full py-1">
                <div>
                  <h3 className="text-xl font-serif font-bold text-[#1b4d3e] mb-2">Predictive Maintenance & Failure Risk</h3>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              <div className="order-2 md:order-1 flex flex-col justify-between h-full py-1">
                <div>
                  <h3 className="text-xl font-serif font-bold text-[#1b4d3e] mb-2">Real-time IoT Sensor Telemetry Stream</h3>
                  <p className="text-xs text-slate-600 leading-relaxed mb-4">
              ...
              <div className="order-1 md:order-2 relative aspect-4/3 rounded-xl overflow-hidden shadow-sm">
            ...
            {/* 2-Card Sub-Grid (Bottom Features) */}
          ...
          <aside className="space-y-8">
            ...
            {/* 1. News Letter Subscription Box */}
            ...
            <div id="categories" className="bg-[#fdfaf7] p-6 rounded-2xl border border-slate-200 shadow-xs">
              ...
              <div className="grid grid-cols-2 gap-5">
                {categoryCircles.map(cat => (
                  <div key={cat.id} className="flex flex-col items-center text-center cursor-pointer group">
                    <div className="w-16 h-16 rounded-full border-2 border-[#1b4d3e] p-0.5 overflow-hidden relative shadow-sm group-hover:scale-105 transition-transform bg-white">
            ...
            <div className="bg-[#1b4d3e] p-6 rounded-2xl text-white text-center shadow-md">
              ...
              <div className="flex items-center justify-center gap-3">
                {/* Facebook */}
                <a href="#" className="w-10 h-10 rounded-full bg-white text-[#1b4d3e] hover:bg-slate-100 flex items-center justify-center font-bold text-sm shadow-sm transition-all hover:scale-110">
                  f
                </a>
                {/* Instagram */}
                <a href="#" className="w-10 h-10 rounded-full bg-white text-[#1b4d3e] hover:bg-slate-100 flex items-center justify-center font-bold text-sm shadow-sm transition-all hover:scale-110">
