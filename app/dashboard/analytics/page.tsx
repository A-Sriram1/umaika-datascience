...
const VIBRANT_COLORS = ['#38bdf8', '#22d3ee', '#34d399', '#fb923c', '#c084fc', '#f472b6', '#facc15'];
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-sky-500/10 via-cyan-500/5 to-purple-500/10 p-6 rounded-3xl border border-sky-500/20 backdrop-blur-xl shadow-xl">
        <div>
          ...
          <p className="text-cyan-200/80 text-sm mt-1 font-medium">Comprehensive manufacturing intelligence across all production lines</p>
        </div>
        <div className="flex gap-3">
          ...
          <button className="flex items-center gap-2 px-5 py-2 rounded-xl bg-gradient-to-r from-sky-500 to-cyan-500 text-xs font-bold text-slate-950 shadow-lg shadow-cyan-500/25 hover:scale-105 transition-transform">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs font-bold px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/30">Target: 85.0%</span>
        </div>
        <ResponsiveContainer width="100%" height={320}>
          <LineChart data={data.weeklyOEE}>
            ...
            <Line type="monotone" dataKey="oee" stroke="#38bdf8" strokeWidth={4} name="Overall OEE" dot={{ fill: '#38bdf8', r: 5 }} />
            <Line type="monotone" dataKey="availability" stroke="#34d399" strokeWidth={2} name="Availability" dot={false} strokeDasharray="4 4" />
            <Line type="monotone" dataKey="performance" stroke="#fb923c" strokeWidth={2} name="Performance" dot={false} strokeDasharray="4 4" />
            <Line type="monotone" dataKey="quality" stroke="#c084fc" strokeWidth={2} name="Quality" dot={false} strokeDasharray="4 4" />
          </LineChart>
        </ResponsiveContainer>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data.machineUtilization} layout="vertical">
              ...
              <Tooltip contentStyle={tooltipStyle} />
              <Bar dataKey="utilization" radius={[0, 8, 8, 0]} fill="#22d3ee" />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
            ...
            {data.productionByShift.map(s => (
              <div key={s.shift} className="rounded-2xl bg-white/[0.03] p-4 border border-white/5">
                ...
                <div className="h-2.5 rounded-full bg-white/10 overflow-hidden p-0.5">
                  <div className="h-full rounded-full bg-gradient-to-r from-sky-400 to-cyan-400" style={{ width: `${(s.output / s.target) * 100}%` }} />
                </div>
                <div className="text-xs font-bold text-cyan-300 mt-1.5 text-right">{s.efficiency}% Efficiency</div>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          className="rounded-3xl bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/90 backdrop-blur-2xl border border-emerald-500/20 p-6 shadow-2xl">
          <h2 className="text-xl font-bold text-white tracking-tight mb-4">Revenue Impact vs Production Cost</h2>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={data.monthlyRevenue}>
              <defs>
                <linearGradient id="revGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#34d399" stopOpacity={0.4} /><stop offset="95%" stopColor="#34d399" stopOpacity={0} /></linearGradient>
                <linearGradient id="costGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={0.4} /><stop offset="95%" stopColor="#f43f5e" stopOpacity={0} /></linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
              ...
              <Legend wrapperStyle={{ color: '#cbd5e1', fontSize: 12, fontWeight: 600 }} />
              <Area type="monotone" dataKey="revenue" stroke="#34d399" strokeWidth={3} fill="url(#revGrad)" name="Revenue Generated" />
              <Area type="monotone" dataKey="cost" stroke="#f43f5e" strokeWidth={3} fill="url(#costGrad)" name="Production Cost" />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          className="rounded-3xl bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/90 backdrop-blur-2xl border border-amber-500/20 p-6 shadow-2xl">
              ...
              return (
                <div key={m.material} className="rounded-2xl bg-white/[0.03] p-3.5 border border-white/5">
                  ...
                  <div className="h-2 rounded-full bg-white/10 relative overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${(m.waste / 8) * 100}%`, backgroundColor: overTarget ? '#f43f5e' : '#34d399' }} />
                  </div>
