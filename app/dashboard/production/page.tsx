...
export default function ProductionPage() {
  ...
  const efficiencyData = useMemo(() => lines.map(l => ({ name: l.name.split(' ').slice(-1)[0], efficiency: l.efficiency, fill: '#38bdf8' })), [lines]);

  const downtimeData = [
    { reason: 'Planned Maintenance', hours: 14.2, fill: '#38bdf8' },
    { reason: 'Mechanical Failure', hours: 11.5, fill: '#f43f5e' },
    { reason: 'Setup/Changeover', hours: 8.3, fill: '#fb923c' },
    { reason: 'Material Shortage', hours: 6.8, fill: '#facc15' },
    { reason: 'Operator Error', hours: 4.2, fill: '#c084fc' },
    { reason: 'Quality Hold', hours: 3.5, fill: '#22d3ee' },
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-sky-500/10 via-cyan-500/5 to-purple-500/10 p-6 rounded-3xl border border-sky-500/20 backdrop-blur-xl shadow-xl">
        <div>
          ...
          <p className="text-cyan-200/80 text-sm mt-1 font-medium">Live production line analytics and throughput metrics</p>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        className="rounded-3xl bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/90 backdrop-blur-2xl border border-cyan-500/20 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-white tracking-tight">Live Cumulative Production Count</h2>
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-300 text-xs font-bold border border-cyan-500/30">
            <Sparkles className="w-3.5 h-3.5" /> Target: {target.toLocaleString()} units
          </div>
        </div>
        <div className="flex items-end gap-6 mb-4">
          <div className="text-6xl font-black bg-gradient-to-r from-sky-400 via-cyan-300 to-emerald-300 bg-clip-text text-transparent font-mono tracking-tight">{counter.toLocaleString()}</div>
        ...
        <div className="h-3.5 rounded-full bg-white/10 overflow-hidden p-0.5 shadow-inner">
          <motion.div className="h-full rounded-full bg-gradient-to-r from-sky-400 via-cyan-400 to-emerald-400 shadow-md shadow-cyan-400/50" initial={{ width: 0 }} animate={{ width: `${Math.min((counter / target) * 100, 100)}%` }} transition={{ duration: 0.5 }} />
        </div>
        <div className="flex justify-between text-xs font-semibold text-slate-400 mt-2.5">
          <span>0 units</span>
          <span className="text-cyan-300 font-bold">{Math.round((counter / target) * 100)}% Shift Target Complete</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="rounded-3xl bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/90 backdrop-blur-2xl border border-sky-500/20 p-6 text-center shadow-xl">
          <div className="text-5xl font-black bg-gradient-to-r from-sky-400 to-cyan-300 bg-clip-text text-transparent mb-1">{oee.overall}%</div>
          <div className="text-xs font-bold text-cyan-300 uppercase tracking-wider">Overall Plant OEE</div>
        </motion.div>
        {oeeGauges.map((g, i) => (
          <motion.div key={g.name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 + i * 0.05 }}
            className="rounded-3xl bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/90 backdrop-blur-2xl border border-white/10 p-4 text-center hover:border-cyan-500/30 transition-all shadow-xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={cycleData}>
              <defs><linearGradient id="cycleGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#22d3ee" stopOpacity={0.4} /><stop offset="95%" stopColor="#22d3ee" stopOpacity={0} /></linearGradient></defs>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
              <XAxis dataKey="timestamp" tick={{ fill: '#94a3b8', fontSize: 11 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fill: '#94a3b8', fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={tooltipStyle} />
              <Area type="monotone" dataKey="value" stroke="#22d3ee" strokeWidth={3} fill="url(#cycleGrad)" />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={efficiencyData}>
              ...
              <Tooltip contentStyle={tooltipStyle} />
              <Bar dataKey="efficiency" radius={[8, 8, 0, 0]} fill="#38bdf8" />
            </BarChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {lines.map(line => {
            ...
            return (
              <div key={line.id} className="rounded-2xl bg-white/[0.03] border border-white/10 p-5 hover:border-cyan-500/40 transition-all shadow-lg">
                ...
                <div className="space-y-2 text-xs font-medium">
                  ...
                  <div className="h-2 rounded-full bg-white/10 overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-sky-400 to-cyan-400" style={{ width: `${Math.min(pct, 100)}%` }} /></div>
                  <div className="grid grid-cols-3 gap-2 pt-2 text-center">
                    ...
                    <div><div className="text-slate-400 text-[10px]">OEE</div><div className="text-cyan-300 font-bold">{line.oee}%</div></div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
        ...
        <h2 className="text-xl font-bold text-white tracking-tight mb-4">Downtime Root Cause Pareto</h2>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={downtimeData} layout="vertical">
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" />
            <XAxis type="number" tick={{ fill: '#94a3b8', fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis type="category" dataKey="reason" tick={{ fill: '#cbd5e1', fontSize: 11 }} axisLine={false} tickLine={false} width={140} />
            <Tooltip contentStyle={tooltipStyle} />
            <Bar dataKey="hours" radius={[0, 8, 8, 0]}>
              {downtimeData.map((d, i) => <Cell key={i} fill={d.fill} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </motion.div>
